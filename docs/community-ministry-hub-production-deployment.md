...
The production App Hosting revision `findyourchurch-palacios-build-2026-08-07-003` serves commit `c79671ba7de2a8c9ab1e21e0d8c654dd7e27e04e`, including the Central-time event-editor fix, premium representative portal/sidebar, and contained square logo previews. Event wall-clock inputs are converted with the IANA zone `America/Chicago` before validation is persisted, including registration-open and registration-close values. The write path was audited through `saveEventEditorSubmission` → `createManagedEvent`/`updateManagedEvent` → the Firestore repository: after validation, `startsAt`, `endsAt`, and registration timestamps are copied without another date conversion.
