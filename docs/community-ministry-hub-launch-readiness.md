...
The production rollout `findyourchurch-palacios-build-2026-08-07-003` serves commit `c79671ba7de2a8c9ab1e21e0d8c654dd7e27e04e`, including the Central-time event-editor conversion, premium representative portal/sidebar, and contained square logo previews. The guarded production acceptance session `20260807052306` passed raw Firestore timestamp checks after event creation and editing, including daylight-saving behavior; the write path performs no second date conversion after validation. A supplementary retry later timed out at the report-email redirect and is retained as an incomplete email follow-up.
...
- The earlier supplementary report-email run `20260807052709` timed out before its success redirect and was not treated as certification. After the acceptance harness was corrected to resolve both canonical and custom-share church routes, guarded production session `20260807115427` passed all 8 of 8 workflow stages, including private PDF/XLSX report generation and report email. The independent exact-cleanup verifier confirmed that no fixture remained.
- Gmail Search Console notice `19fdb0d944778087` reported two non-critical Event structured-data suggestions: missing `performer` and `offers`. Commit `83bc820` adds both fields to event JSON-LD, with a `PerformingGroup` performer and an `Offer` containing the canonical event URL, USD currency, availability, and cost description. Static tests and the deployed staging JSON-LD assertions passed; production App Hosting revision `findyourchurch-palacios-build-2026-08-07-004` is healthy at 100% traffic.
- Search Console has not yet re-crawled the corrected production pages. The current Gmail account could not open the property UI, so the owner must grant property access (or use the verified owner account), inspect the live URL, and request validation. Do not claim Google validation until that recrawl completes.
- Historical GitHub failure notices for this repository are stale/deleted runs. Current GitHub evidence shows `Validate build` run `31067259443` and scheduled `Annual Listing Verification` run `31118597052` completed successfully on `main`; no current CI failure is open.
