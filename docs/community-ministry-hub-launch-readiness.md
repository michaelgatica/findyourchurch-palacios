...
The production Rules API readback was unavailable to the current CLI identity with HTTP 403. Do not infer rule deployment from repository files or staging evidence. The release operator must use an identity authorized to deploy/read production rules, verify project `findyourchurch-24562` and database `findyourchurchpal`, deploy the reviewed `firestore.rules` and `storage.rules`, and immediately run deployed access checks before registrations open.
