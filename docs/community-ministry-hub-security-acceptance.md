...
| Controlled public smoke | `/churches`, `/submit`, `/contact`, `/portal/login`, `/admin/login`, `/privacy`, and `/terms` rendered without browser errors at 375px and had no horizontal overflow. After all 25 production indexes reached `READY`, `/`, `/events`, and a real church profile also returned meaningful content without the prior Server Components error. | Public index-related blocker closed. Authenticated representative/admin smoke remains pending the production rollout. |
