...
**NO-GO for production deployment today.** Provider-backed staging email, production monitoring, operational-record retention, Firestore recovery controls, and Storage soft-delete recovery are certified. Production remains blocked by the owner's required native screen-reader test, explicit acceptance or remediation of 9 remaining moderate dependency advisory nodes, first managed Firestore backup/restore evidence, App Check enforcement after a valid production token exchange, production secret-manager binding, and passing production smoke tests. Required production indexes are now ready.
...
| SMTP provider | Owner confirmation identifies Namecheap Stellar Plus. DNS exposes SPF, default-selector DKIM, and DMARC `p=none`; MX routes to the configured hosting mail service. | Namecheap documents 200 messages/hour/domain and 100 recipients/message for Stellar Plus. The supplied noreply credential returned SMTP `535 EAUTH`; production binding and live bounce verification remain blocked until the mailbox credential is corrected privately. |
| Controlled public smoke | `/churches`, `/submit`, `/contact`, `/portal/login`, `/admin/login`, `/privacy`, and `/terms` rendered without browser errors at 375px and had no horizontal overflow. After all 27 production indexes reached `READY`, `/`, `/events`, and a real church profile also returned meaningful content without the prior Server Components error. | Public index-related blocker closed. Authenticated representative/admin smoke remains pending the production rollout. |
| Secret storage | Five production secrets now hold the Firebase client key, registration token, export signing key, Scheduler token, and listing-verification token. App Hosting access was granted and plaintext backend overrides were removed. `apphosting.yaml` references production Secret Manager names only. | Noreply SMTP password secret remains intentionally absent because the supplied credential failed authentication. |
