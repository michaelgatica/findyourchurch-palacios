...
export async function sendSubmissionChangesRequestedNotification(input: {
  ...
  await sendTransactionalEmail({
    ...
    body: [
      ...
      input.adminMessage,
      "",
      "Please respond with the needed information or submit a corrected request. Once the information is complete, we will review it again as soon as possible.",
      "",
      ...getMissionNote(),
    ].join("\n"),
    relatedEntityType: "churchSubmission",
...
export async function sendClaimApprovedNotification(input: {
  ...
  await sendTransactionalEmail({
    ...
    body: [
      ...
      "",
      `Church team: ${createEmailLink("Invite an authorized representative", buildAbsoluteUrl("/portal/team"))}`,
      "",
...
export async function sendRepresentativeUpdateChangesRequestedNotification(input: {
  ...
  await sendTransactionalEmail({
    ...
    body: [
      ...
      input.adminMessage,
      "",
      `Please open the editor, make the requested changes, and save again: ${createEmailLink("Edit requested changes", buildAbsoluteUrl("/portal/church/edit"))}`,
      "",
      "Your previously submitted information will be loaded in the editor, so you only need to adjust the fields requested by the review team. Once the information is complete, we will review it again as soon as possible.",
      "",
      ...getMissionNote(),
    ].join("\n"),
    relatedEntityType: "churchUpdateRequest",
