                  ...
                  <div className="registration-builder-fields">
                    {section.fields.map((field, fieldIndex) => (
                      <div key={field.id} className="registration-builder-field">
                              ...
                              return (
                                <div key={participantField.id} className="registration-participant-builder__field">
                                  ...
                                  <div className="button-row">
                                    <button type="button" className="button button--ghost button--small" disabled={participantIndex === 0} onClick={() => updateParticipantFields(section.id, field.id, (participantFields) => { const fields = [...participantFields]; [fields[participantIndex - 1], fields[participantIndex]] = [fields[participantIndex], fields[participantIndex - 1]]; return fields; })}>Move up</button>
                                    <button type="button" className="button button--ghost button--small" disabled={participantIndex === siblingFields.length - 1} onClick={() => updateParticipantFields(section.id, field.id, (participantFields) => { const fields = [...participantFields]; [fields[participantIndex], fields[participantIndex + 1]] = [fields[participantIndex + 1], fields[participantIndex]]; return fields; })}>Move down</button>
                                    <button type="button" className="button button--ghost button--small" onClick={() => updateParticipantFields(section.id, field.id, (participantFields) => [...participantFields.slice(0, participantIndex + 1), { ...structuredClone(participantField), id: `participant-${crypto.randomUUID()}` }, ...participantFields.slice(participantIndex + 1)])}>Duplicate</button>
                                    <button type="button" className="button button--danger button--small" disabled={siblingFields.length === 1} onClick={() => updateParticipantFields(section.id, field.id, (participantFields) => participantFields.filter((candidate) => candidate.id !== participantField.id))}>Remove</button>
                                  </div>
                                </div>
                              );
                            })}
                        ...
                        <div className="button-row">
                          <button type="button" className="button button--ghost button--small" disabled={fieldIndex === 0} onClick={() => moveField(section.id, fieldIndex, -1)}>Move up</button>
                          <button type="button" className="button button--ghost button--small" disabled={fieldIndex === section.fields.length - 1} onClick={() => moveField(section.id, fieldIndex, 1)}>Move down</button>
                          <button type="button" className="button button--ghost button--small" onClick={() => setSections((current) => normalizeOrder(current.map((candidate) => candidate.id === section.id ? { ...candidate, fields: [...candidate.fields.slice(0, fieldIndex + 1), { ...structuredClone(field), id: `field-${crypto.randomUUID()}` }, ...candidate.fields.slice(fieldIndex + 1)] } : candidate)))}>Duplicate</button>
                          <button type="button" className="button button--danger button--small" onClick={() => setSections((current) => normalizeOrder(current.map((candidate) => candidate.id === section.id ? { ...candidate, fields: candidate.fields.filter((candidateField) => candidateField.id !== field.id) } : candidate)))}>Remove</button>
                        </div>
                      </div>
                    ))}
                  </div>
                  <div className="button-row">
                    <button type="button" className="button button--ghost button--small" onClick={() => setSections((current) => normalizeOrder(current.map((candidate) => candidate.id === section.id ? { ...candidate, fields: [...candidate.fields, createField()] } : candidate)))}>Add field</button>
                    {sections.length > 1 ? <button type="button" className="button button--danger button--small" onClick={() => setSections((current) => normalizeOrder(current.filter((candidate) => candidate.id !== section.id)))}>Remove section</button> : null}
